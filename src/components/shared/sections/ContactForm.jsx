...
const sanitizeformData = (emailData) => {
  const errors = {};
  ...
  if (!emailData.company_name || emailData.company_name.trim().length < 2) {
    errors.company_name = "Company name must be at least 2 characters long.";
  }

  // Validate designation
  if (!emailData.designation || emailData.designation.trim().length < 2) {
  ...
  if (!emailData.source_path || emailData.source_path.trim() === "") {
...
export const ContactForm = () => {
  ...
  const onSubmit = async (e) => {
    ...
    const query = await fetch(`/api/verifyToken?token=${token}`)
    ...
    if (response.data) {
      ...
      if (isValidForm) {
        ...
        let res = sanitizeformData(emailData)
        if (res.isValid) {
          // console.log(res)
